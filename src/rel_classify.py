...
class EmbeddingModel:
    def __init__(self, model_path: str, device_id: int = 7):
        from sentence_transformers import SentenceTransformer
        
        self.device = f"cuda:{device_id}" if torch.cuda.is_available() else "cpu"
        # This one line does everything correctly
        self.model = SentenceTransformer(
            model_path,
            device=self.device,
            trust_remote_code=True,      # required for NV-Embed-v1/v2
            model_kwargs={"torch_dtype": torch.bfloat16}  # optional, slightly faster on H200
        )

    def encode(self, texts: List[str]) -> torch.Tensor:
        # Returns normalized embeddings directly on the correct GPU
        embeddings = self.model.encode(
            texts,
            normalize_embeddings=True,
            batch_size=32,           # tweak if you want
            show_progress_bar=False,
            convert_to_tensor=True
        )
        return embeddings  # already on self.device and normalized
...
class LocalLLMClassifier:
    ...
    def __init__(
        ...
        load_kwargs = dict(
            ...
            device_map="balanced",        # Automatically shard model across allowed GPUs
