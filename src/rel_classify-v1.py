...
# --- 1. IMPORTS (MUST RUN FIRST) ---
import torch
from nltk.stem import WordNetLemmatizer
from nltk.stem import PorterStemmer

# Import transformers components immediately so we can patch them
...
# Try importing Mistral to patch it; if not present, ignore
try:
...
# =============================================================================
# --- 2. COMPATIBILITY PATCHES (MUST RUN SECOND) ---
# =============================================================================

# PATCH A: Fix 'MISTRAL_INPUTS_DOCSTRING' missing in transformers >= 4.45
# The cached NV-Embed code looks for this string, but new transformers removed it.
if "transformers.models.mistral.modeling_mistral" in locals() or \
   "transformers.models.mistral.modeling_mistral" in sys.modules:
    # Re-check import just to be safe
    import transformers.models.mistral.modeling_mistral
...
# PATCH B: Fix missing attributes in Custom Configs (transformers >= 4.46)
# The NV-Embed custom config class misses 'dtype' and '_attn_implementation'.
# We inject a fallback that provides safe defaults for these missing keys.
def _patched_getattr(self, key):
    if key == "dtype":
        return torch.float32
    if key == "_attn_implementation":
        # New transformers versions check this to decide between SDPA/FlashAttn/Eager.
        # "eager" is the safest fallback for older custom models.
        return "eager"
    
    # If the key is something else, raise the standard error
    raise AttributeError(f"'{type(self).__name__}' object has no attribute '{key}'")

# Apply the patch to the base configuration class
...
class EmbeddingModel:
    ...
    def encode(self, texts: List[str]) -> torch.Tensor:
        ...
        # CRITICAL FIX: Move input tensors to self.device (e.g., GPU 7).
...
class LocalLLMClassifier:
    ...
    def call(self, prompt: str, max_new_tokens: int = 16) -> str:
...
def ensemble_decision(
    ...
    logger.info("s_embed time: %.4f", (s_embed_time_et - s_embed))
    ...
    if llm is not None:
        ...
        logger.info("nli time: %.4f", llm_time_e - nli_time_e)
